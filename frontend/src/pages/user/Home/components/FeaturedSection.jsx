...
import { ArrowRight, Timer, TrendingUp, Loader2 } from 'lucide-react';
...
export default function FeaturedSection() {
    ...
    useEffect(() => {
        const fetchFeatured = async () => {
            try {
                ...
                const auctionList = response.data || response; // Handle Laravel pagination
    ...
    if (loading) return (
        <div className="flex flex-col items-center justify-center py-20 text-gray-500">
            <LoadingSpinner height="h-20" size="h-8 w-8" color="border-[#D70B27]" />
            <p className="font-semibold tracking-wide uppercase text-sm mt-4">Loading top picks...</p>
        </div>
    );
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                {items[0] && (
                    ...
                    >
                        <img src={getImageUrl(items[0].asset?.media?.thumbnail)} alt={items[0].asset?.title} className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition duration-700" />
                ...
                {items[1] && (
                    ...
                    >
                        <img src={getImageUrl(items[1].asset?.media?.thumbnail)} alt={items[1].asset?.title} className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition duration-700" />
                ...
                {items[2] && (
                    ...
                    >
                        <img src={getImageUrl(items[2].asset?.media?.thumbnail)} alt={items[2].asset?.title} className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition duration-700" />
                ...
                {items[3] && (
                    ...
                    >
                        <img src={getImageUrl(items[3].asset?.media?.thumbnail)} alt={items[3].asset?.title} className="absolute inset-0 w-full h-full object-cover opacity-60 group-hover:scale-105 transition duration-700" />
                        ...
                        <div className="absolute top-1/2 -translate-y-1/2 left-8 max-w-sm">
                            ...
                            <p className="text-gray-400 text-sm mb-6 line-clamp-2">
                                A rare opportunity to acquire this masterpiece. 
